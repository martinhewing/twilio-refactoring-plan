...
const MODULES = [
  {
    ...
    color: "#f59e0b",
    weeks: "Gate 1",
    description: "No code is touched until the debugger halts on a breakpoint and curl returns 200. Vocabulary must be established before observing the live system.",
    sections: [
      {
        id: "00-A",
        title: "Part A — Environment Setup",
        description: "Get the full stack running locally. Every item is a hard prerequisite for Part B.",
        tasks: [
          {
            id: "00-A-1",
            ...
            subtasks: [
              ...
              { id: "00-A-1b", label: "cd twilio && git log --oneline -10 — read recent commit history" },
              { id: "00-A-1c", label: "Identify the most recent circular-import hotfix in the log" },
            ],
          },
          {
            id: "00-A-2",
            ...
            subtasks: [
              ...
              { id: "00-A-2b", label: "source .venv/bin/activate (or .venv\\Scripts\\activate on Windows)" },
              { id: "00-A-2c", label: "Confirm prompt shows (.venv)" },
            ],
          },
          {
            id: "00-A-3",
            ...
            subtasks: [
              ...
              { id: "00-A-3b", label: "pip install pytest pytest-asyncio httpx -- no additional deps yet" },
              { id: "00-A-3c", label: "python -c 'import fastapi, redis, twilio, anthropic' -- confirm all import clean" },
            ],
          },
          {
            id: "00-A-4",
            ...
            subtasks: [
              ...
              { id: "00-A-4e", label: "Confirm .env is in .gitignore -- check for committed credentials (Security gate)" },
              { id: "00-A-4f", label: "If credentials are committed: rotate them now, before proceeding" },
            ],
          },
          {
            id: "00-A-5",
            ...
            subtasks: [
              { id: "00-A-5a", label: "docker run -d -p 6379:6379 --name twilio-redis redis:7-alpine" },
              { id: "00-A-5b", label: "redis-cli ping -- must return PONG" },
              { id: "00-A-5c", label: "Optional: open RedisInsight and connect to localhost:6379" },
            ],
          },
          {
            id: "00-A-6",
            ...
            subtasks: [
              { id: "00-A-6a", label: "uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload" },
              { id: "00-A-6b", label: "Confirm 'Uvicorn running on http://0.0.0.0:8000' in terminal" },
              { id: "00-A-6c", label: "curl http://localhost:8000/health -- must return 200" },
              { id: "00-A-6d", label: "Open http://localhost:8000/docs -- Swagger UI must load" },
            ],
          },
          {
            id: "00-A-7",
            ...
            subtasks: [
              { id: "00-A-7a", label: "Run -> Edit Configurations -> + -> Python" },
              { id: "00-A-7b", label: "Script path: output of 'which uvicorn'" },
              { id: "00-A-7c", label: "Parameters: app.main:app --host 0.0.0.0 --port 8000" },
              { id: "00-A-7d", label: "Working directory: project root" },
              { id: "00-A-7e", label: "Environment variables: copy all from .env" },
              { id: "00-A-7f", label: "Click Debug (bug icon) -- confirm server starts in PyCharm console" },
              { id: "00-A-7g", label: "Set a breakpoint in whatsapp_webhook() -- line with 'async def whatsapp_webhook'" },
              { id: "00-A-7h", label: "Verify breakpoint dot turns solid red (not hollow)" },
            ],
          },
          {
            id: "00-A-8",
            ...
            subtasks: [
              { id: "00-A-8a", label: "ngrok http 8000 -- in a separate terminal" },
              ...
              { id: "00-A-8d", label: "Set webhook URL to https://xxxxx.ngrok.io/whatsapp-webhook, method POST" },
              { id: "00-A-8e", label: "Save configuration" },
            ],
          },
          {
            id: "00-A-9",
            label: "Verify the full pipeline",
            subtasks: [
              { id: "00-A-9a", label: "Send 'hello' from personal WhatsApp to the Twilio sandbox number" },
              { id: "00-A-9b", label: "Confirm PyCharm console shows a log line" },
              { id: "00-A-9c", label: "Confirm ngrok shows POST /whatsapp-webhook 200" },
              { id: "00-A-9d", label: "Confirm PyCharm debugger halts on the breakpoint in whatsapp_webhook()" },
              { id: "00-A-9e", label: "Checkpoint PASSED: debugger halts + 200 response. Do not proceed until both are true." },
            ],
          },
        ],
      },
      {
        id: "00-B",
        title: "Part B — Vocabulary & Principles",
        description: "Before observing the live system, establish the classification vocabulary. You cannot name what you see in Module 01 unless these are internalised first.",
        tasks: [
          {
            id: "00-B-1",
            label: "Method category taxonomy (CAT labels)",
            subtasks: [
              { id: "00-B-1a", label: "CAT-1 Getter / CAT-2 Setter: no logic, single attribute" },
              { id: "00-B-1b", label: "CAT-3 Predicate: returns bool, no side effects" },
              { id: "00-B-1c", label: "CAT-4 Factory: creates and returns, must have no side effects" },
              { id: "00-B-1d", label: "CAT-5 Validation: raises on bad input, no side effects" },
              { id: "00-B-1e", label: "CAT-6 Orchestration: coordinates other methods -- the core violation in whatsapp_webhook()" },
              { id: "00-B-1f", label: "CAT-7 Mutation: modifies internal state" },
              { id: "00-B-1g", label: "Can you classify initialize_user_session()? (Answer before Module 01)" },
            ],
          },
          {
            id: "00-B-2",
            label: "Pattern taxonomy (PAT labels)",
            subtasks: [
              { id: "00-B-2a", label: "PAT-4 Facade: single entry point hiding subsystem complexity" },
              { id: "00-B-2b", label: "PAT-5 Cache-aside: check cache, on miss fetch and populate" },
              { id: "00-B-2c", label: "PAT-8 Stateful dispatch: FSM-driven routing, not if/elif chains" },
              { id: "00-B-2d", label: "Which pattern does whatsapp_webhook() currently violate by being an FSM + I/O + AI combined?" },
            ],
          },
          {
            id: "00-B-3",
            label: "Protocol / repository taxonomy (PRT labels)",
            subtasks: [
              { id: "00-B-3a", label: "PRT-2 Repository Protocol: single source of truth for a data entity" },
              { id: "00-B-3b", label: "PRT-4 Fake repository: in-memory stand-in for testing -- must satisfy Protocol" },
              { id: "00-B-3c", label: "Why does the monolith have three Redis clients rather than one PRT-2?" },
            ],
          },
          {
            id: "00-B-4",
            label: "SOLID principles as applied to this codebase",
            subtasks: [
              { id: "00-B-4a", label: "SRP: a function has one reason to change. whatsapp_webhook() has at least four. Name them." },
              { id: "00-B-4b", label: "LSP: a fake/concrete must be substitutable for the Protocol. All repos in refactor must pass an LSP test suite." },
              { id: "00-B-4c", label: "DIP: depend on abstraction (Protocol), not concrete (redis.StrictRedis). Identify every concrete dep in main.py." },
            ],
          },
          {
            id: "00-B-5",
            label: "Concurrency fault taxonomy (CONC labels)",
            subtasks: [
              { id: "00-B-5a", label: "CONC-3: sync IO inside async handler -- serialises the FastAPI event loop" },
              { id: "00-B-5b", label: "Identify every redis.StrictRedis call inside an async def in main.py" },
              { id: "00-B-5c", label: "CONC-5: unprotected shared mutable state under concurrent access" },
              { id: "00-B-5d", label: "What happens if two POST /whatsapp-webhook arrive for the same customer simultaneously?" },
            ],
          },
          {
            id: "00-B-6",
            label: "Characterization test rule: Understand -> Observe -> Test",
            subtasks: [
              { id: "00-B-6a", label: "Rule: never write an assertion for behaviour you have not observed live" },
              { id: "00-B-6b", label: "Rule: no assert_called_with, no call_count -- these are wiring tests, not behaviour tests" },
              { id: "00-B-6c", label: "Rule: test the session shape in Redis after every webhook call, not just the HTTP response code" },
              { id: "00-B-6d", label: "Rule: if a bug cannot be fixed yet, write the test that documents the broken behaviour and mark it xfail" },
            ],
          },
          {
            id: "00-B-7",
            label: "Pre-module comprehension gate (answer in writing before starting 01)",
            subtasks: [
              { id: "00-B-7a", label: "What CAT label is whatsapp_webhook()? Why?" },
              { id: "00-B-7b", label: "Name the four SRP concerns inside whatsapp_webhook()" },
              { id: "00-B-7c", label: "What is CONC-3 and which specific call in main.py triggers it?" },
              { id: "00-B-7d", label: "What is the difference between a wiring test and a characterization test?" },
              { id: "00-B-7e", label: "What is the three-client Redis problem and which files own each client?" },
            ],
          },
        ],
      },
